...
export const Bonus = () => {
    ...
    console.log('token', token, 'bonus', bonus)

    const dateparser = (dateBurning: string) => {
        ...
        if (token) {
            ...
            fetch(`api/v3/ibonus/generalinfo/${token}`, { headers })
                ...
                .then(data => setBonus({
                    ...
                    dateBurning: data.data.dateBurning.slice(5, 7) + '.' + data.data.dateBurning.slice(8, 10),
    ...
    return(
        <section className={styles.bonus}>
            <div className={styles.bonus__card}>
                <h2 className={styles.card__title}>{bonus?.currentQuantity} бонусов</h2>
                <p className={styles.card__description}>   {bonus?.dateBurning} сгорит 
                    ...
                    {bonus?.forBurningQuantity} бонусов</p>
